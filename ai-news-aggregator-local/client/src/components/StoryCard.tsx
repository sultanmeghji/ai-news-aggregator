...
import { ExternalLink, Copy, Check, Smile, CheckCircle, RotateCcw, Share2, TrendingUp } from "lucide-react";
...
export default function StoryCard({ story }: StoryCardProps) {
  ...
  const socialPostMutation = useMutation({
    ...
    onSuccess: (data, variables) => {
      toast({
        ...
        description: `Your post has been shared to ${variables.platform}`,
      });
    },
  ...
  const handleSocialPost = (platform: 'linkedin' | 'twitter') => {
    ...
    <div className={`bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-shadow duration-200 ${getCardBorderColor(story.category)}`}>
      <div className="p-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            <Button
              onClick={() => handleSocialPost('linkedin')}
            ...
            <Button
              onClick={() => handleSocialPost('twitter')}
              disabled={socialPostMutation.isPending}
